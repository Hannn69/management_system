...
interface UpdateStatusLabelContentProps {
  slug: string;
}

export function UpdateStatusLabelContent({ slug }: UpdateStatusLabelContentProps) {
  ...
  const fetchStatusLabel = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase}/status-labels/${slug}`, { credentials: "include" });
      if (res.ok) {
      ...
      console.error("Failed to fetch status label", err);
    } finally {
      setFetching(false);
    }
  }, [apiBase, slug]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${apiBase}/status-labels/${slug}`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || "Failed to update status label");
      }

      push("Status label updated successfully!", "success");
    ...
    <main className="px-6 pb-6 pt-5">
      <div className="mx-auto w-full max-w-[1380px] flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground tracking-tight">Update Status Label</h2>
            <p className="text-zinc-500 dark:text-zinc-400 text-sm">Modify workflow state details for {form.name}.</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-zinc-500 ml-1">Status Name</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-zinc-500 ml-1">Status Type</label>
                  <div className="relative">
