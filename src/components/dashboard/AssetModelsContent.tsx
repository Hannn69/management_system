...
import { ChevronDown, MoreHorizontal, Search, Package } from "lucide-react";
...
export function AssetModelsContent() {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to load asset models.");
      ...
      */
      ...
      const allRecords = [
        { id: 1, name: "MacBook Pro 14", slug: "macbook-pro-14", code: "MBP14", isActive: true, createdAt: new Date().toISOString() },
        { id: 2, name: "Dell XPS 15", slug: "dell-xps-15", code: "XPS15", isActive: true, createdAt: new Date().toISOString() },
        { id: 3, name: "iPhone 13 Pro", slug: "iphone-13-pro", code: "IP13P", isActive: true, createdAt: new Date().toISOString() },
        { id: 4, name: "Logitech MX Master 3", slug: "mx-master-3", code: "MXM3", isActive: false, createdAt: new Date().toISOString() },
        { id: 5, name: "Samsung Odyssey G7", slug: "odyssey-g7", code: "OG7", isActive: true, createdAt: new Date().toISOString() },
      ];
    ...
    <main className="px-6 pb-6 pt-5">
      <div className="mx-auto w-full max-w-[1380px] flex flex-1 flex-col gap-6 rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.02)] backdrop-blur">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-zinc-400">Settings / Asset Models</p>
            <h2 className="text-lg font-semibold text-zinc-100">Asset Models</h2>
          </div>
          <button 
            onClick={() => router.push("/asset-models/create")}
            className="rounded-lg bg-amber-600 px-4 py-2 text-xs font-semibold text-white hover:bg-amber-500 shadow-lg shadow-amber-900/20"
          >
            Add Asset Model
          </button>
        </div>
          ...
          <Table className="min-w-[800px]">
            <TableHeader className="bg-white/5">
              <TableRow className="border-white/10">
                <TableHead className="px-4 py-3 text-zinc-100">Model Name</TableHead>
                <TableHead className="px-4 py-3 text-zinc-100">Model Number</TableHead>
            ...
            <TableBody className="divide-y divide-white/10">
              {decoratedRecords.map((record) => (
                <TableRow key={record.id} className="border-white/10 transition-colors hover:bg-white/5">
                  ...
                  <TableCell className="px-4 py-3">
                    <span className="rounded-md border border-white/5 bg-white/5 px-2 py-1 text-xs font-mono text-zinc-300">
                      {record.code || "-"}
                    </span>
                  </TableCell>
