...
import { ChevronDown, MoreHorizontal, Search, MapPin } from "lucide-react";
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      */
      ...
      const allRecords = [
        { id: 1, name: "New York Office", slug: "new-york-office", isActive: true, createdAt: new Date().toISOString() },
        { id: 2, name: "London Studio", slug: "london-studio", isActive: true, createdAt: new Date().toISOString() },
        { id: 3, name: "Tokyo Branch", slug: "tokyo-branch", isActive: true, createdAt: new Date().toISOString() },
        { id: 4, name: "Berlin Hub", slug: "berlin-hub", isActive: false, createdAt: new Date().toISOString() },
        { id: 5, name: "Remote", slug: "remote", isActive: true, createdAt: new Date().toISOString() },
      ];
    ...
    <main className="px-6 pb-6 pt-5">
      <div className="mx-auto w-full max-w-[1380px] flex flex-1 flex-col gap-6 rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.02)] backdrop-blur">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-zinc-400">Settings / Locations</p>
            <h2 className="text-lg font-semibold text-zinc-100">Locations</h2>
          </div>
          <button 
            onClick={() => router.push("/locations/create")}
            className="rounded-lg bg-cyan-600 px-4 py-2 text-xs font-semibold text-white hover:bg-cyan-500 shadow-lg shadow-cyan-900/20"
          >
            Add Location
          </button>
        </div>
          ...
          <Table className="min-w-[700px]">
            <TableHeader className="bg-white/5">
              <TableRow className="border-white/10">
                <TableHead className="px-4 py-3 text-zinc-100">Location Name</TableHead>
