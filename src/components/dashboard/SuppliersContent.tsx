...
import { ChevronDown, MoreHorizontal, Search, Truck } from "lucide-react";
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      */
      ...
      const allRecords = [
        { id: 1, name: "Amazon Business", slug: "amazon-business", isActive: true, createdAt: new Date().toISOString() },
        { id: 2, name: "CDW", slug: "cdw", isActive: true, createdAt: new Date().toISOString() },
        { id: 3, name: "Newegg", slug: "newegg", isActive: true, createdAt: new Date().toISOString() },
        { id: 4, name: "B&H Photo Video", slug: "bh-photo", isActive: false, createdAt: new Date().toISOString() },
        { id: 5, name: "Best Buy Business", slug: "best-buy", isActive: true, createdAt: new Date().toISOString() },
      ];
    ...
    <main className="px-6 pb-6 pt-5">
      <div className="mx-auto w-full max-w-[1380px] flex flex-1 flex-col gap-6 rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.02)] backdrop-blur">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-zinc-400">Settings / Suppliers</p>
            <h2 className="text-lg font-semibold text-zinc-100">Suppliers</h2>
          </div>
          <button 
            onClick={() => router.push("/suppliers/create")}
            className="rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-500 shadow-lg shadow-blue-900/20"
          >
            Add Supplier
          </button>
        </div>
          ...
          <Table className="min-w-[700px]">
            <TableHeader className="bg-white/5">
              <TableRow className="border-white/10">
                <TableHead className="px-4 py-3 text-zinc-100">Supplier Name</TableHead>
